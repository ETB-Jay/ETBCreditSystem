...
const Prompt = ({ children }) => (
    <div className="fixed inset-0 flex items-center justify-center z-10 backdrop-blur-[2px] transition-normal select-none">
        <div className="flex flex-col items-center justify-center rounded-xl bg-gray-900 h-fit w-2/3 md:w-7/12 lg:w-1/2 xl:w-1/3 p-4 border border-gray-700 shadow-xl prompt-animate">
            {children}
        </div>
    </div>
);
...
const PromptField = ({ label, children, error=undefined }) => (
    <div className="relative flex flex-col w-full mb-3">
        <div className="flex flex-row items-center gap-4">
            <p className="text-gray-200 font-semibold text-sm min-w-[100px] text-right select-none prompt-animate">{label}</p>
            <div className="flex-1">
                {children}
            </div>
        </div>
        {error ? <div className="text-red-400 text-xs ml-1 select-none">{error}</div> : null}
    </div>
);
...
const PromptInput = ({ value, onChange, type = 'text', step, disabled, name, placeholder }) => (
    <input
        className="flex justify-center items-center disabled:bg-gray-200 disabled:cursor-not-allowed text-gray-200 text-sm bg-gray-800 rounded-md px-2 py-0.5 h-7 w-full outline-none transition-colors focus:bg-gray-500 hover:bg-gray-700 border border-gray-700 placeholder-gray-500"
        type={type}
        step={step}
        value={value || ''}
        onChange={onChange}
