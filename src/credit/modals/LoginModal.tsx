      ...
      <>
        {autofill.map((autofillContent) => (
          <div
            ...
            className="theme-text gray-bg-medium gray-hover cursor-pointer p-1 transition-colors"
...
const LoginModal = (): ReactElement => {
  ...
  const handleAutofillSelect = useCallback(
    (entry: AutofillProps) => {
      ...
      setShowAutofill(false);
      void handleSignIn(entry.email, entry.password);
    },
    [handleSignIn]
  );
  ...
  return (
    <Modal title="Login" icon={<LockIcon sx={{ width: 16, height: 16 }} />} closeable={false}>
      ...
      >
        <ArrowDropDownIcon className="theme-text hover:brightness-75" />
        <AutofillDropdown
          autofill={autofill}
