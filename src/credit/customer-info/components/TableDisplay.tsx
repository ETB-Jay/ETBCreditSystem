import { useEffect, useState, useMemo, type ReactElement, type KeyboardEvent } from "react";
...
const TableDisplay = (): ReactElement => {
  ...
  return (
    <div
      className={cn(
        "container-snap overflow-y-auto rounded-xl border",
      ...
      <table className="lg:text-md theme-text w-full text-xs select-none md:text-sm">
        ...
        <tbody className="divide-y divide-gray-800">
          {filteredRows.map((row) => (
            <tr
              key={
                typeof row.date === "object" && "seconds" in row.date
                  ? row.date.seconds
                  : row.date.toString()
              }
